...
export function AgentView({ session, organization }: { session: Session; organization: Organization }) {
  ...
  return <section className="workspace"><ViewHeader eyebrow={organization.name} title="Agente" description="Tono, preguntas frecuentes y políticas operativas." />{error && <ViewError>{error}</ViewError>}
    <form className="workspace-card agent-editor" onSubmit={save}>{config ? <><label>Tono de atención<textarea rows={5} value={config.tone || ''} onChange={event => field('tone', event.target.value)} /></label><label>Preguntas frecuentes<textarea rows={10} value={config.faq || ''} onChange={event => field('faq', event.target.value)} /></label><label>Políticas<textarea rows={8} value={config.policies || ''} onChange={event => field('policies', event.target.value)} /></label><div className="save-row"><span>{saved ? 'La nueva información ya está disponible para el agente.' : `Configuración activa · versión ${config.version}`}</span><button disabled={busy}>{busy ? 'Guardando…' : 'Guardar configuración'}</button></div></> : <p className="quiet-empty">No existe una configuración activa para esta organización.</p>}</form>
  </section>
}
